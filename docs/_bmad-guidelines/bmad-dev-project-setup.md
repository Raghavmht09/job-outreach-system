...
## 📋 Prerequisites: Manual Setup for supabase project setup and other things should be completed by user

**BEFORE using this ask user to verify below:**
- [ ] Manual setup completed as mentioned in file /docs/_bmad-guidelines/bmad-architect-ux-setup.md
- [ ] Supabase database schema created (jobs, connections, messages tables)
- [ ] Environment variables configured (`.env.local`)
- [ ] Auth bypassed (`NEXT_PUBLIC_BYPASS_AUTH=true`)
- [ ] shadcn/ui components installed
- [ ] TypeScript types generated (`types/supabase.ts`)

**If setup incomplete:** check what steps are missing and , Stop and ask complete setup first.

---

## 🧹 Project Context: Vercel Supabase Starter Cleanup

**What You're Working With:**

This project was bootstrapped with Vercel's official Supabase starter template. Here's what's already configured and what to ignore:

### ✅ Files to KEEP (Already Configured)

| File | Purpose | Do NOT Modify |
|------|---------|---------------|
| `lib/supabase/server.ts` | Supabase server client (for Server Components) | ✅ Keep as-is |
| `lib/supabase/client.ts` | Supabase client client (for Client Components) | ✅ Keep as-is |
| `middleware.ts` | Auth bypass logic (replaced during setup) | ✅ Keep as-is |
| `.env.local` | Environment variables | ✅ Keep as-is |
| `types/supabase.ts` | Generated database types | ✅ Regenerate if schema changes |

### 🚫 Files/Folders to IGNORE (Don't Touch)

| File/Folder | Why Ignore | Action |
|-------------|------------|--------|
| `app/login/` | Auth page (bypassed for MVP) | Ignore, don't delete |
| `app/signup/` | Auth page (bypassed for MVP) | Ignore, don't delete |
| `app/auth/` | Auth callbacks (not needed) | Ignore, don't delete |

**Why not delete?** These files contain working auth patterns you can reference later when enabling auth. Deleting them is unnecessary and wastes time.

### 🆕 Files You'll CREATE (Your Work)

| File | Purpose | When to Create |
|------|---------|----------------|
| `app/jobs/page.tsx` | Job URL Input page | First feature |
| `app/connections/[jobId]/page.tsx` | Connections List page | Second feature |
| `components/jobs/JobUrlForm.tsx` | Job URL input form | First component |
| `components/connections/ConnectionCard.tsx` | Connection display card | Second component |
| `lib/validations/job.ts` | Zod schemas for job validation | With first API route |
| `app/api/jobs/route.ts` | API route for jobs CRUD | First backend work |

### 🔧 Current Environment (From Manual Setup)
...
- ✅ All database operations use stub user ID
- ✅ RLS policies allow access via stub user ID
- ✅ You can test features immediately without auth flow

**When auth is enabled later:**
1. Change `NEXT_PUBLIC_BYPASS_AUTH=false`
2. Remove default user_id from database schema
3. Auth just works (starter template patterns already in place)
...
- **RLS Verification:** Test that User A cannot see User B's data (even with bypass)
